...
public class CircuitoParaleloUI extends JFrame {
    ...
    public CircuitoParaleloUI() {
        setTitle("Módulo de Análisis de Circuitos en Paralelo");
        setSize(750, 750); 
    ...
    private void configurarEventos(JPanel panelAcciones) {
        ...
        btnCalcular.addActionListener(e -> {
            try {
                ...
                double conductanciaTotal_entradas = 0;
                double resistenciaTotal_entradas = 0;

                // 1. Manejar las resistencias si existen
                if (!camposResistencias.isEmpty()) {
                    for (int i = 0; i < camposResistencias.size(); i++) {
                        ...
                        conductanciaTotal_entradas += (1.0 / valorR);
                    }
                    
                    if (conductanciaTotal_entradas == 0) {
                         JOptionPane.showMessageDialog(this, "Error: Las resistencias ingresadas resultan en Rt infinita.", "Error Matemático", JOptionPane.ERROR_MESSAGE);
                         return;
                    }
                    resistenciaTotal_entradas = 1.0 / conductanciaTotal_entradas;
                }

                // 2. Lógica de cálculo (Ley de Ohm: V = I * R)
                ...
                // CASO 3: I conocido y (V o R desconocida) -> Necesita Rt de las entradas
                    ...
                    // Lógica de Transformación de Fuente (usando Rt)
                    mensajeTransformacion = String.format(
                        "*** Se realizó una TRANSFORMACIÓN DE FUENTE: ***\n" +
                        "Fuente de Corriente (I=%.6f A en paralelo con Rt=%.6f Ω)\n" +
                        "Convertida a Fuente de Voltaje (V=%.6f V en serie con Rt=%.6f Ω).\n\n",
                        corriente, resistenciaTotal_calculada, voltaje, resistenciaTotal_calculada);
        ...
        btnIA.addActionListener(e -> {
            ...
            try {
                String datosCircuito = String.format(
                    "Circuito Paralelo: Voltaje=%.6f V, Corriente=%.6f A, Resistencia total=%.6f Ω.", 
                    ultimoVoltaje, ultimaCorriente, ultimaResistenciaTotal);

                ChatBotUI chatWindow = new ChatBotUI("Paralelo", datosCircuito);
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new CircuitoParaleloUI().setVisible(true));
    }
}